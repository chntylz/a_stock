...
def showImageInHTML(imageTypes,savedir):
    ...
    for image in images:
        ...
        if is_exist_quad(stock_name, 30): 
            ...
            quad_list.append(image)
            print('quad stock_name:%s' % stock_name)
        else:
            print('!!! none quad stock_name:%s' % stock_name)
            
    print('quad_list=%s' % quad_list)
    ...
    ret_df = comm_generate_web_dataframe(real_dir, quad_list, curr_day, dict_industry)

    comm_handle_html_head(newfile_2, stock_data_dir, curr_day )
    day_continue_handle_html_body_special(newfile_2, curr_day)
    comm_handle_html_body(newfile_2, ret_df)
    day_continue_handle_html_end_special(newfile_2, dict_industry)
