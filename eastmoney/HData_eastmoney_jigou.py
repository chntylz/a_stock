...
class HData_eastmoney_jigou(object):
    ...
    def table_is_exist(self):
        self.db_connect()
        self.cur.execute("select count(*) from pg_class where relname = 'eastmoney_jigou_table' ;")
    ...
    def db_hdata_xq_create(self):
        ...
        # 创建stocks表
        self.cur.execute('''
            drop table if exists eastmoney_jigou_table;
            create table eastmoney_jigou_table(
		...
		PJZF float,
            ...
            alter table eastmoney_jigou_table add primary key(stock_code, record_date, index);
            ''')
        self.conn.commit()
        self.db_disconnect()

        print("db_eastmoney_jigou_table_create finish")
        pass
    ...
    def copy_from_stringio(self, df):
        ...
        self.db_connect()
        try:
            self.cur.copy_from(buffer, table='eastmoney_jigou_table', sep=",")
            self.conn.commit()
    ...
    def db_get_maxdate_of_stock(self,stock_code):#获取某支股票的最晚日期
        ...
        self.db_connect()
        self.cur.execute("select max(record_date) from eastmoney_jigou_table \
    ...
    def get_data_from_hdata(self, stock_code=None, 
        ...
        sql_temp += "select * from eastmoney_jigou_table"
    ...
    def delete_data_from_hdata(self, stock_code=None, 
        ...
        sql_temp = "delete from eastmoney_jigou_table"
...
#alter table eastmoney_jigou_table add  "up_days" int not null default 0;


#update eastmoney_jigou_table set is_zig=0 where record_date = '2018-10-08' and stock_code = '002732';

#UPDATE eastmoney_jigou_table SET is_zig = tmp.is_zig, is_quad=tmp.is_quad FROM    (VALUES ( DATE  '2018-06-13', '600647', 11, 1), ( DATE  '2018-06-12', '600647', 12, 1) ) AS tmp (record_date, stock_code, is_zig, is_quad ) WHERE eastmoney_jigou_table.record_date = tmp.record_date and eastmoney_jigou_table.stock_code = tmp.stock_code;
#select * from eastmoney_jigou_table where stock_code ='600647' and (record_date='2018-06-13' or record_date='2018-06-12' ); 
