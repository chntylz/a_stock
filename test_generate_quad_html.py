...
def quad_continue_handle_html_body_special(newfile, date):
    f = newfile
    curr_day = date
...
def quad_continue_handle_html_end_special(newfile, dict_industry):
    with open(newfile,'a') as f:
        f.write('\n')
        f.write('\n')
        f.write('\n')
...
def showImageInHTML(imageTypes,savedir):
    ...
    quad_continue_handle_html_end_special(newfile, dict_industry)
